...
if [ $stage -le 2 ]; then
  utils/fix_data_dir.sh ${trn_set} || eixt 1
  utils/fix_data_dir.sh ${dev_set} || exit 1
  utils/fix_data_dir.sh ${tst_set} || exit 1

  local/augment_data.sh ${trn_set}
fi

trn_set=${trn_set}_augment

# MFCC plus pitch features.
if [ $stage -le 3 ]; then
  for x in train_augment dev test; do
