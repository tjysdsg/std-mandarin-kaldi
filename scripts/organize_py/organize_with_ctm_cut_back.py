...
def sig_index_to_poi_index(sig_beg, sig_end):
...
def cut_word_and_save(items):
    ...
    tbeg = round(sr * float(tbegin))
    tend = round(sr * float(tend))
...
def get_words_list(ctm_file):
    ...
    for utt_id in content_dict.keys():
        ...
        word_segments.append([utt_id, "nihaomiya", float(content["nihao"][2]), float(content["miya"][2]) + float(content["miya"][3])])
