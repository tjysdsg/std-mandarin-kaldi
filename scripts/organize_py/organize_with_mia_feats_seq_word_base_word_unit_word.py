...
def cut_word_and_save(items):
    ...
    #     fea_beg += 1
    win = 8
    while len(feats[fea_mid - win - 1: fea_mid + win]) < 17:
        win += 1
...
def get_words_list(ctm_file):
    ...
    for utt_id in content_dict.keys():
        ...
        #else:
        word_segments.append([utt_id, "ni", float(content["你"][2]) + float(content["你"][3])/2 ])
        word_segments.append([utt_id, "hao", float(content["好"][2]) + float(content["好"][3])/2 ])
        word_segments.append([utt_id, "mi",  float(content["米"][2]) + float(content["米"][3])/2 ])
        word_segments.append([utt_id, "ya",  float(content["雅"][2]) + float(content["雅"][3])/2 ])
