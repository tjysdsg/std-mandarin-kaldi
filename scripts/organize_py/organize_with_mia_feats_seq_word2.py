...
def cut_word_and_save(items):
    ...
    #     fea_beg += 1
    win = 20
    while len(feats[fea_mid - win : fea_mid + win]) < 40:
        win += 1
    # print(items, fea_mid - win, fea_mid + win)
    feats = feats[fea_mid - win : fea_mid + win]
    feats = feats[0:40]
...
def get_words_list(ctm_file):
    ...
    for utt_id in content_dict.keys():
        ...
        word_segments.append([utt_id, "nihao", float(content[1][2]) + float(content[1][3]) ])
        word_segments.append([utt_id, "haomi", float(content[3][2]) + float(content[3][3]) ])
        word_segments.append([utt_id, "miya",  float(content[5][2]) + float(content[5][3]) ])
