...
def get_words_list(ctm_file):
    ...
    for index, items in tqdm(read_file_gen(ctm_file)):
        ...
        if items[4] in content_dict[items[0]].keys():
            content_dict[items[0]][items[4] + "another"] = items
        else:
    ...
    for utt_id in content_dict.keys():
        ...
        word_segments.append([utt_id, "wanother",  float(content["wanother"][2]) + float(content["wanother"][3])/2])
        word_segments.append([utt_id, "enanother",  float(content["enanother"][2]) + float(content["enanother"][3])/2])
        """
        word_segments.append([utt_id, "h",  float(content["h"][2]) + float(content["h"][3])/2])
